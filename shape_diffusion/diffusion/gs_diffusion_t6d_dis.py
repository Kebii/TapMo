...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l1_loss = th.nn.L1Loss(reduce="none")

        self.mean = torch.from_numpy(np.load('../datasets/HumanML3D/Mean_30p_6d.npy')).float()
        self.std = torch.from_numpy(np.load('../datasets/HumanML3D/Std_30p_6d.npy')).float()

        self.handle_map = [[2,11],[11,26],[26,27], [13,8],[8,19],[19,18], [7,23],[23,29], [1,3],[3,25]]
        self.skeleton = [[0,18],[18,19],[19,8],[8,13],
                         [0,27],[27,26],[26,11],[11,2],
                         [0,14],[14,25],[25,3],[3,1],
                         [0,20],[20,29],[29,23],[23,7],
                         [0,24],[24,4]]
    ...
    def masked_l2(self, a, b, mask):
        ...
        loss = sum_flat(loss * (1.0-mask))  # gives \sigma_euclidean over unmasked elements
        n_entries = a.shape[1] * a.shape[2]
        non_zero_elements = sum_flat((1.0-mask)) * n_entries
    ...
    def arap_loss(self, edge_index, x1, x2, weights=None):
    ...
    def temporal_reg_loss(self, sample, mask):
        bs, d, _, T = sample.shape
        temp_var = sample[:,:,:, 1:] - sample[:,:,:, :T-1]
        temp_var = temp_var.reshape(bs, 41, -1, T-1)
        temp_var = temp_var[:, :40, 3:, :]
        reg_loss = 1e-6/ (temp_var*1e6 + 1e-16)
        mask = (1.0 - mask.float())                                    # bs seq_len
        mask = mask[:,:-1].unsqueeze(1).unsqueeze(1)           # bs 1 1 seq_len-1
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output, output_mod = model(x, self._scale_timesteps(t), **model_kwargs)
        ...
        x = x.reshape(model_mean.shape)
        model_log_variance = model_log_variance.reshape(model_mean.shape)
        assert (
    ...
    def p_sample(
        ...
        noise = noise.reshape(out["mean"].shape)
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "modify":out["modify"]}
    ...
    def p_sample_with_grad(
        ...
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"].detach(), "modify":out["modify"]}
    ...
    def p_sample_loop(
        ...
        for i, sample in enumerate(self.p_sample_loop_progressive(
            ...
            if dump_steps is not None and i in dump_steps:
                ...
                dump_modify.append(deepcopy(sample["modify"]))
            final = sample
        if dump_steps is not None:
            return dump, dump_modify
        
        return final["sample"], final["modify"]
    ...
    def get_gen_loss(self, fake_score):
        bceloss = nn.BCELoss(reduction='none')
        gen_motion_loss = bceloss(fake_score, torch.ones(fake_score.shape).cuda(fake_score.device))
        gen_motion_loss = torch.mean(gen_motion_loss)
        return gen_motion_loss
    
    def get_dis_loss(self, real_score, fake_score):
        bceloss = nn.BCELoss(reduction='none')
        gen_motion_loss = bceloss(fake_score, torch.zeros(fake_score.shape).cuda(fake_score.device))
        gen_motion_loss = torch.mean( gen_motion_loss)

        dis_motion_loss = bceloss(real_score, torch.ones(real_score.shape).cuda(real_score.device))
        dis_motion_loss = torch.mean(dis_motion_loss)

        return gen_motion_loss + dis_motion_loss

    def training_losses(self, model, x_start, x_theta, t, discriminator, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output, model_output_modi = model(x_t, self._scale_timesteps(t), **model_kwargs)

            bs, num_joint, d, T = model_output.shape
            model_output = model_output.reshape(bs, num_joint*d, 1, T)
        ...
        mean = self.mean.to(model_output.device).reshape(-1)[None, :, None, None]
        std = self.std.to(model_output.device).reshape(-1)[None, :, None, None]
        model_output_inv = model_output * std + mean
        # target_inv = target * std + mean

        char_handle = model_kwargs['y']['char_handle'].to(model_output.device)               # bs 40 3
        char_handle_pos, char_handle_t = trans2hdpos_(model_output_inv.clone().detach(), model_output_modi, char_handle, True)
        handle_0 = [self.handle_map[i][0] for i in range(len(self.handle_map))]
        handle_1 = [self.handle_map[i][1] for i in range(len(self.handle_map))]

        char_handle_pos_ = char_handle_pos.reshape(bs, T, num_joint-1, -1)
        char_handle_t_ = char_handle_t.reshape(bs, T, num_joint-1, -1)                                               # bs T -1  --> bs T 30 3
        
        char_handle_pos_0 = char_handle_pos_[:,:, handle_0, :]
        char_handle_pos_1 = char_handle_pos_[:,:, handle_1, :]
        char_handle_t_0 = char_handle_t_[:,:, handle_0, :]
        char_handle_t_1 = char_handle_t_[:,:, handle_1, :]

        handle_dis = torch.sum((char_handle_pos_1 - char_handle_pos_0)**2, dim=-1)
        t_dis = torch.sum((char_handle_t_1 - char_handle_t_0)**2, dim=-1)

        handle_dis_v = handle_dis[:, 1:, :] - handle_dis[:, :-1, :]

        handle_dis = handle_dis.permute(0, 2, 1).contiguous().reshape(bs, -1, 1, T)
        t_dis = t_dis.permute(0, 2, 1).contiguous().reshape(bs, -1, 1, T)
        loss_handle = self.masked_l1(t_dis, handle_dis, mask) + mean_flat(handle_dis_v**2)

        terms["handle_mse"] = 1e-4*loss_handle

        # ---------------------------------------------------------------------------
        terms["modi_reg"] = 1e-4 * torch.sum(model_output_modi**2)

        if self.lambda_gen>0:

            # mesh discrimator
            focus_verts = model_kwargs['y']['focus_verts'].to(model_output.device)
            focus_skin = model_kwargs['y']['focus_skin'].to(model_output.device)
            tranf = model_output_inv.clone().detach().reshape(bs, num_joint, d, T)
            tranf[:, :, :3, :] = tranf[:, :, :3, :] + model_output_modi
            output_focus_verts = handle2mesh(tranf, char_handle, focus_skin, focus_verts)
            char_feature = model_kwargs['y']['char_feature']
            score_fake = discriminator(output_focus_verts, char_feature)
            gen_loss = self.get_gen_loss(score_fake)
            terms["gen_loss"] = gen_loss
        

        terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) + terms.get('handle_mse', 0.) + terms.get('modi_reg', 0.) + self.lambda_gen*terms.get('gen_loss', 0.)

        return terms, output_focus_verts.detach()
...
def get_mesh(x, model):
    '''
    x: B N 3+4 T
    '''
    return model.trans2mesh(x)

def get_handle(x, model):
    return model.trans2mesh.trans2hdpos(x)

def get_handle_(x, model):
    return model.trans2mesh.trans2hdpos_(x)

def theta_lbs(theta, model):
    # theta B J 3
    return model.trans2mesh.theta_lbs(theta)
...
def trans2hdpos(transformation, handle_pos):
        ...
        hd_disp = disp     # B*T K 3
        hd_rot = rot        # B*T K 3 3
        hd_pos = handle_pos.to(device).unsqueeze(1).repeat(1, T, 1, 1).reshape(bs*T, K, 3)      # B*T K 3

        per_hd_v = torch.einsum("abcd,abd->abc", hd_rot, hd_pos) + hd_disp  # (B*T, K, 3)
        per_hd_v = per_hd_v.reshape(bs, T, -1).contiguous()
        hd_pos = hd_pos.reshape(bs, T, -1).contiguous()

        return per_hd_v, hd_pos

def trans2hdpos_(transformation, modification, handle_pos, if_modi=False):
        device = transformation.device
        B, K, _ = handle_pos.shape   # bs 40 3
        bs, d, _, T = transformation.shape
        transformation = transformation.reshape(bs, K+1, 9, T).permute(0,3,1,2).contiguous().reshape(bs*T, K+1, 9)[:, :K, :]
        modification = modification.permute(0,3,1,2).contiguous().reshape(bs*T, K+1, 3)[:, :K, :]
        disp = transformation[:, :, :3]         # B K 3
        ...
        if if_modi:
            per_hd_v = hd_pos + hd_disp + modification  # (B*T, K, 3)
        else:
            per_hd_v = hd_pos + hd_disp
        per_hd_v = per_hd_v.reshape(bs, T, -1).contiguous()
        hd_pos = hd_pos.reshape(bs, T, -1).contiguous()
...
def handle2mesh(transformation, handle_pos, region_score, v0):
    ...
    transformation: (B, 40, 3+6, T)
    handle_pos: handle position of T-pose  (B, 30, 3)
    region_score: (B, V, 30)
    v0: (B, V, 3)
    """
    B, K, _, T = transformation.shape
    V = v0.shape[1]
    transformation = transformation.permute(0,3,1,2).contiguous().reshape(B*T, K, _)
    disp = transformation[:, :K-1, :3]
    rot = transformation[:, :K-1, 3:]
    rot = repr6d2mat_tensor(rot)
    # rot = quaternion_to_rotation_matrix(rot.view(B*K, 4).contiguous(), order=wxyz).view(B, K, 3, 3).contiguous()
    hd_disp = disp[:, None, :, :]                            # BT, 1 30, 3
    hd_pos = handle_pos[:, None, :, :].repeat(T, 1, 1, 1)    # BT, 1 30, 3
    hd_rot = rot[:, None, :,:,:].repeat(1, V, 1, 1, 1)                             # BT, 1 30 3 3
    v0 = v0[:, :, None, :].repeat(T, 1, 1, 1)                    # BT, V 1 3
    region_score = region_score[:, :, :, None].repeat(T, 1, 1, 1)

    per_hd_v = torch.einsum("abcde,abce->abcd", hd_rot, (v0 - hd_pos)) + hd_pos + hd_disp
    v = torch.sum(region_score * per_hd_v, 2)  # (BT, V, 3)
