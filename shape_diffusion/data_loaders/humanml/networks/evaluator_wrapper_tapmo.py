...
class EvaluatorModelWrapper(object):
    ...
    def __init__(self, opt):
        ...
        if opt.dataset_name == 't6d_mixrig':
            opt.dim_pose = 279
...
def build_evaluators(opt):
    ...
    #                         map_location=opt['device'])
    checkpoint = torch.load(pjoin("/apdcephfs/private_jiaxuzhang/code/text-to-motion/saves/t2m", 'text_mot_match_30p6d', 'model', 'finest.tar'),
                            map_location=opt['device'])
    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
...
class EvaluatorMDMWrapper(object):
    ...
    def __init__(self, dataset_name, device):
        opt = {
            ...
            'dim_pose': 279 if dataset_name == 't6d_mixrig' else 251,
