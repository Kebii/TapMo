...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs, theta, posed_handle, posed_verts) in tqdm(enumerate(dataloader)):
                ...
                for t in range(repeat_times):
                    ...
                    if t == 0:
                        sub_dicts = [{'motion': sample[bs_i].reshape((-1, sample[bs_i].shape[2])).squeeze().permute(1,0).cpu().numpy(),
                    ...
                    if is_mm:
                        mm_motions += [{'motion': sample[bs_i].reshape((-1, sample[bs_i].shape[2])).squeeze().permute(1,0).cpu().numpy(),
                                        'length': model_kwargs['y']['lengths'][bs_i].cpu().numpy(),
                                        } for bs_i in range(dataloader.batch_size)]
