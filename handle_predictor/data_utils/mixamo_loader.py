...
class MixamoDataset(Dataset):
    ...
    def __init__(self, data_dir, flag=None, preload=True,
                 ...
                 log_prob=False, joint_augmentation=False):
        ...
        if flag:
            with open(os.path.join(data_dir, f'{flag}_char_split.txt')) as f:
    ...
    def get(self, index):
        ...
        if self.joint_augmentation:
            p = np.random.rand()
            if p > 0.2:
                scale = np.random.randint(80, 150)
                aug_joints0_1 = joint_augment(aug_joints0_1, scale)
                aug_joints0_2 = joint_augment(aug_joints0_2, scale)
            else:
                aug_joints0_1 = aug_joints0_1
                aug_joints0_2 = aug_joints0_2
    ...
    def _preload_motion(self):
        ...
        if self.flag:
            with open(os.path.join(self.data_dir, f'{self.flag}_motion_split.txt')) as f:
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        sequence_containing_x_vals = data1.v0[:,0].numpy()
        sequence_containing_y_vals = data1.v0[:,1].numpy()
        sequence_containing_z_vals = data1.v0[:,2].numpy()

        xp = data1.joints[0][:,0]
        yp = data1.joints[0][:,1]
        zp = data1.joints[0][:,2]

        print(data1.joints[0].shape)

        ax.scatter(xp, yp, zp, s=20)
        ax.scatter(sequence_containing_x_vals, sequence_containing_y_vals, sequence_containing_z_vals, s=2)

        bone_lst = [[0,1], [0,2], [0,3], [1,4], [2,5], [3,6], [4,7], [5,8], [6,9], [7,10], [7,11], [7,12], [8,13], [9,14], [10,15],[11,16],[12,17], [16,18],[17,19],[18,20],[19,21]]

        for i, bone in enumerate(bone_lst):
            ax.plot(np.hstack((xp[bone[0]], xp[bone[1]])),
            np.hstack((yp[bone[0]], yp[bone[1]])),
            np.hstack((zp[bone[0]], zp[bone[1]])),
            ls='-', linewidth=3)

        plt.savefig('./work_dir/mixamo.jpg')
        save_obj('./work_dir/mixamo.obj', data1.v1.numpy(), data1.triangle[0])
